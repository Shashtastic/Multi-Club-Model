...
if other_clubs_in_region <= 2:
    if np.mean(adjusted_attendance) <= 0.9:
        club_adjusted_attendance = adjusted_attendance * 1.075 
    else: 
        club_adjusted_attendance = adjusted_attendance * 1.05
...
total_social_media_followers = 300 # e.g., 787 for 787k followers
...
transfer_outgoing = 0.5
def transfers_outgoing():

    if transfer_outgoing > 5:
