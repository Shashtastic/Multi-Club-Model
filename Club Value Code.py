...
other_clubs_in_region = float(input("Enter number of clubs in city: ")) 
...
g_trends_average = float(input("Enter average value of google trends over past 5 years for club's name: "))
...
follower_norm = np.full(num_simulations, (np.log(adjusted_followers) - np.log(50)) / (np.log(1000) - np.log(50)))
...
def youth_emergence_probability(other_clubs_in_region, population_multiplier):
    ...
    p_emerge_variable = p_emerge_base * population_multiplier
...
transfer_outgoing = float(input("Enter average value of transfer outgoings: ")) 
def transfers_outgoing():
    if transfer_outgoing > 15:
        mean_ln_in, sigma_ln_in = 3.5, 0.5
    elif transfer_outgoing > 7.5:
        mean_ln_in, sigma_ln_in = 2.8, 0.5
...
simulated_values = (np.array([simulate_academy_product() for _ in range(num_simulations)]))*city_score
...
# STADIUM CAPACITY
capacity = float(input("Enter stadium capacity: "))

def stadium_cap_weight():
    thresholds = [
        (10000, 0.4), (12500, 0.5), (15000, 0.55), (17500, 0.6), (20000, 0.65),
...
# LEAGUE STABILITY
league_stability = float(input("Enter standard deviation in league position: "))
...
# MANAGER STABILITY
no_managers = float (input("Enter number of permanent managers in last 3 years: "))
...
score_weight = 0.25
adjusted_stad_cap_weight = 0.1
manager_stability_multiplier_weight = 0.1
...
operating_income = float(input("Enter the average operating income in thousand: "))
payroll = float(input("Enter average payroll in thousands: "))
profit_before_tax = float(input("Enter average profit(loss) before tax: "))
max_profit_before_tax = float(input("Enter max profit(loss) before tax: "))
min_profit_before_tax = float(input("Enter min profit(loss) before tax: "))
